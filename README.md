...
P0의 비교 모집단을 서울 25개 자치구로 한정한 것은 지역 수를 줄이기 위해서가 아니라 **비교 단위를 통일하기 위해서**입니다.

- 인천·광주는 최근 행정구역 통합 이슈로 기준 단위가 바뀌어, 코드에서 지역 코드를 잘못 매핑하는 오류가 날 위험이 있어 제외했습니다.
- 부산·대구·울산 등 광역시는 자치구와 군이 함께 있어, 한 분포 안에서 percentile을 비교하면 행정단위의 동질성이 깨집니다. 그래서 전국 60여 개 시군구로 넓히지 않았습니다.
- 서울은 25개가 모두 자치구여서 동일한 행정단위끼리 통계를 낼 수 있습니다.
- 동일한 방법론은 다른 지역의 구 단위 모집단에도 그대로 적용할 수 있습니다.
