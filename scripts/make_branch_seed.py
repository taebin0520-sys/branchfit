...
# 파일명은 config.DATASET_VERSION에서 끌어옵니다.
...
LOCALITY_NAMES: dict[str, list[str]] = {
    ...
    "용산구": ["한남", "이태원", "후암", "효창", "원효로", "청파", "서빙고", "용산"],
    ...
    "강북구": ["미아", "수유", "번동", "우이"],
    "도봉구": ["창동", "방학", "쌍문", "도봉"],
    "노원구": ["상계", "중계", "하계", "공릉", "월계", "노원"],
    "은평구": ["불광", "연신내", "응암", "수색", "구산"],
    "서대문구": ["홍제", "연희", "신촌", "충정로", "홍은"],
    "마포구": ["공덕", "아현", "합정", "망원", "상암", "성산", "서교", "도화"],
...
#
# '몇 번째 점포'로 지정하지 않고 '어느 자치구의 마지막 점포'로 지정합니다.
# 순번으로 고정하면 자치구별 점포 수가 줄어들 때 그 순번이 사라져
# 출장소가 조용히 4곳 미만으로 줄어듭니다(v3->v4 교체 때 중구 13->12로 실제 발생).
SUB_OFFICE_REGIONS = {"강남구", "중구", "영등포구", "구로구"}
...
def build_rows(regions: list[dict[str, str]]) -> list[dict[str, object]]:
    ...
    for region in regions:
        ...
        for seq in range(1, count + 1):
            # 해당 자치구의 마지막 점포 1곳만 출장소로 표시
            is_sub = name in SUB_OFFICE_REGIONS and seq == count
...
def main() -> None:
    ...
    # 출장소가 4곳이 아니면 SUB_OFFICE_REGIONS 지정이 깨진 것이므로 즉시 실패시킵니다.
    if sub != len(SUB_OFFICE_REGIONS):
        raise AssertionError(f"출장소 수 불일치: {sub} != {len(SUB_OFFICE_REGIONS)}")
